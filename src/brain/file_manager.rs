...
impl FileManager {
    ...
    fn mover(&self, config: &ConfigZelpes, arquivo: &Path) {
        let file = arquivo.file_name().unwrap();
        let nome: &str = file.to_str().unwrap();

        let destino = match self {
            FileManager::LocalCofre => {
                logger::registrar(&config.logs, "movido para pasta Cofre", nome);
                config.cofre.join(file)
            }
            FileManager::LocalBackup => {
                logger::registrar(&config.logs, "movido para pasta backup", nome);
                config.backup.join(file)
            }
            FileManager::LocalLogs => {
                logger::registrar(&config.logs, "log movido para o TMP", nome);
                config.tmp.join(file)
            }
            FileManager::LocalTmp => {
                logger::registrar(&config.logs, "movido para tmp", nome);
                config.tmp.join(file)
            }
        };
        fs::create_dir_all(destino.parent().unwrap()).ok();
        fs::rename(arquivo, &destino).expect("[ERRO] ao mover arquivo para {destino}");
    }
}
